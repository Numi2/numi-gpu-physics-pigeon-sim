...
func silhouetteHoleAuditDistinguishesEnclosedAndExteriorBackground() {
  ...
  let packedIdentities = closed.map { $0 ? UInt32(0x1F02) : UInt32(0) }
  ...
  #expect(enclosed.adjacentPackedIdentities == [0x1F02])
...
func silhouetteAuditPreservesRetractedPedalSpaceAcrossWingCoverts() {
  ...
  let primaryCovertClassified = CrowShowcaseFrame.silhouetteHoles(
    birdMask: bird,
    featherClassCodes: classes,
    width: width,
    height: height
  )
  #expect(primaryCovertClassified.pixelCount == 0)
  #expect(primaryCovertClassified.expectedLowerBodyAperturePixelCount == 6)
}

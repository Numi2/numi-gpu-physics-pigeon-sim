...
The schema-13 JSON report records format and coordinate conventions plus per-frame
...
anatomical owners. For the largest enclosed component, schema `13` also
...
Schema `13` additionally records projected-size ventral-barb candidate and
frustum-visible record counts, indirectly expanded vertex count, and reserved
output-capacity bytes. These are executable work/residency counters, not a GPU
speed claim; duration still requires a controlled repeated benchmark.
